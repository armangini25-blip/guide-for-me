...
//
// Build 2026-08-29 (achado crítico — ver comentário gêmeo em pista.js): este número TAMBÉM precisa
// ser copiado manualmente pro "?v=" de `<script src="assets/js/app.js?v=...">` e
...
// Build 2026-08-28 (Auditoria 05, achado alto A-03): chamada de novo a cada selectLanguage() (troca
// de idioma é um fluxo normal, tem botão dedicado) — sem a guarda de idempotência, cada troca
// religava um listener novo em cima dos elementos [data-voice] sem remover os anteriores, então da
// 2ª troca de idioma em diante cada hover disparava o mesmo áudio de UI várias vezes sobrepostas.
// Mesmo padrão de dataset já usado em attachArPauseSync, só não tinha sido aplicado aqui.
function attachHoverVoice() {
  $all("[data-voice]").forEach((el) => {
    if (el.dataset.hoverVoiceAttached) return;
    el.dataset.hoverVoiceAttached = "1";
...
function playTrack(tipo) {
  ...
  if (pistaCarouselTimer) { clearInterval(pistaCarouselTimer); pistaCarouselTimer = null; }
  if (tipo1StoryboardTimer) { clearInterval(tipo1StoryboardTimer); tipo1StoryboardTimer = null; }
  // Build 2026-08-28 (Auditoria 05, achado médio M-06): faltava cancelar a Web Animation do sweep
  // em si — os 3 timers acima já eram limpos, mas sair de um mirante em pleno sweep direto pra uma
  // tela sem sweep próprio (ex. pista_convite) deixava a animação anterior rodando por cima da foto
  // pelos segundos restantes da duração. pista.js já tinha isso certo em pararStoryboardAR().
...
function verificarSeguidor() {
  ...
  if (!state.presenterFollows) {
    box.innerHTML = `
      ...
      <a class="btn block" href="https://www.instagram.com/parquebondinho/" target="_blank" rel="noopener noreferrer">${t("brinde_seguir_link")}</a>
  ...
  setTimeout(() => {
    box.innerHTML = `
      ...
      <a class="btn secondary block" href="assets/brinde/brinde.jpg" download="Pao-de-Acucar-para-Voce-brinde.jpg" target="_blank" rel="noopener noreferrer">${t("brinde_baixar_imagem")}</a>
